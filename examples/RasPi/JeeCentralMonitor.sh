...
echo `date "+%Y/%m/%d %X"` $node $command $type $badCRC $sequence $attempts $Voltage $SalusID $SalusCommand $SalusNoise $ColdFeed $BoilerFeed $CHreturn $TCreturn $temperature >> /etc/heyu/JeeCentralMonitor.dat
...
#################################################################
wget -q -O- "https://api.thingspeak.com/update?key=O64IA4X1ZIVU9DWN&field4=$Voltage&field5=$ColdFeed&field6=$BoilerFeed&field7=$CHreturn&field8=$TCreturn&field1=$temperature&field2=$humidity" > /dev/null 2>&1
#