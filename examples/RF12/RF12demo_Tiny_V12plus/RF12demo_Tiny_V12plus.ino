...
typedef struct {
    ...
    byte output       :2;   // 0 = dec, 1 = hex, 2 = hex+ascii
...
// allowable frequencies and their use when selecting operating frequencies.
        ...
        case 't': // broadcast a maximum size test packet, request an ack
            ...
            Serial.print(RF69::interruptCount);// TODO DEBUG
            printOneChar(',');
            Serial.print(GIMSK);// TODO DEBUG
...
void loop () {
    ...
    if (rf12_recvDone()) {
        ...
        if (config.output > 1) { // also print a line as ascii
            showString(PSTR("ASC "));
            if (config.group == 0) {
                showString(PSTR(" II "));
            }
