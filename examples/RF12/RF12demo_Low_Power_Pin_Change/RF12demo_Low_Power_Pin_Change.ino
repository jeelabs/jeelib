...
static void handleInput (char c) {
    ...
    if (((byte) c > 239) || (c == '.')) {      // Wake up looses some bits
        showString(PSTR(".\r"));
        return;
    }
...
#if PINCHG_IRQ            
            unsigned int busy = Sleepy::idleSomeTime(TERMINAL_TIMEOUT);  // 5 seconds timeout for keyboard activity
